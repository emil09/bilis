...
function getActiveList(coo_no){
	$.ajax({
		url: 'activetripslist/active_trips_list',
		type: 'post',
		data: {coo_no: coo_no},
		success: function(data, status) {
			...
			$('#table-activetripslist').dataTable().fnDestroy();
			$("#active_list_data").html(table_data);
			var tabler = $('#table-activetripslist').DataTable({ // height: 837px
				paging : false,
				scrollY: '45vh',
    			scrollCollapse: true,
				scrollX: true,
			...
			$('#selectallrows').click(function(){
		    	$('#table-activetripslist tbody tr').addClass('DTTT_selected selected');
		    });
			$('#deselectallrows').click(function(){
		    	$('#table-activetripslist tbody tr').removeClass('DTTT_selected selected');
		    });
