...
function get_active_list(coo_no){
	$.ajax({
		...
		success: function(data, status){
			...
			$('#table-salesbydriver').dataTable().fnDestroy();
		    $('#active_list_data').html(table_data);
			var tabler = $('#table-salesbydriver').DataTable({
				paging : false,
				// autoWidth : false,
				// scrollY: '45vh',
				// scrollCollapse: true,
			...
			$('#selectallrows').click(function(){
		    	$('#table-salesbydriver tbody tr').addClass('DTTT_selected selected');
		    });
			$('#deselectallrows').click(function(){
		    	$('#table-salesbydriver tbody tr').removeClass('DTTT_selected selected');
		    });
	    }
	});
}
